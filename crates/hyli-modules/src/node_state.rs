...
use hyli_tld::{handle_blob_for_hyli_tld, validate_hyli_contract_blobs};
...
impl NodeState {
    ...
    fn handle_blob_tx(
        ...
        let blobs: BTreeMap<BlobIndex, UnsettledBlobMetadata> = tx
            ...
            .filter_map(|(index, blob)| {
                ...
                {
                    let hyli_output = hyli_verifiers::native::verify(
                        blob_tx_hash.clone(),
                        BlobIndex(index),
                        &tx.blobs,
                        verifier,
                    );
                    tracing::trace!("Native verifier in blob tx - {:?}", hyli_output);
                    ...
                    if hyli_output.success {
                        ...
                        return Some((
                            ...
                            UnsettledBlobMetadata {
                                blob: blob.clone(),
                                possible_proofs: vec![(verifier.into(), hyli_output)],
                            },
                        ));
                    }
    ...
    fn handle_blob_proof(
        ...
        block_under_construction
            ...
            .push(TransactionStateEvent::NewProof {
                blob_index: blob_proof_data.hyli_output.index,
                proof_tx_hash: proof_tx_hash.clone(),
                program_output: blob_proof_data.hyli_output.program_outputs.clone(),
            });
        ...
        block_under_construction
            ...
            .push(HandledBlobProofOutput {
                ...
                contract_name: unsettled_tx.blobs[&blob_proof_data.hyli_output.index]
                    .blob
                    .contract_name
                    .clone(),
                hyli_output: blob_proof_data.hyli_output.clone(),
            });
    ...
    fn on_settled_blob_tx(
        ...
        if !txs_to_nuke.is_empty() {
            ...
            for (tx_hash, hyli_outputs) in txs_to_nuke.iter() {
                if let Some(unsettled_blob_tx) = self.unsettled_transactions.get(tx_hash) {
                    for hyli_output in hyli_outputs {
                        if let Some(blob_metadata) = unsettled_blob_tx.blobs.get(&hyli_output.index)
                        {
                            let contract_name = &blob_metadata.blob.contract_name;
                            if let Some(contract) = self.contracts.get(contract_name) {
                                let mut hyli_output = hyli_output.clone();
                                // If the hyli_output received is failed, we select the current state of the contract as the initial and next state
                                if !hyli_output.success {
                                    hyli_output.initial_state = contract.state.clone();
                                    hyli_output.next_state = contract.state.clone();
                                }
                                updates
                                    .entry(tx_hash.clone())
                                    .or_default()
                                    .push((contract.program_id.clone(), hyli_output.clone()));
                            } else {
            ...
            for (tx_hash, hyli_outputs) in updates {
                if let Some(unsettled_blob_tx) = self.unsettled_transactions.get_mut(&tx_hash) {
                    for (program_id, hyli_output) in hyli_outputs {
                        if let Some(blob_metadata) =
                            unsettled_blob_tx.blobs.get_mut(&hyli_output.index)
                        {
                            // This is a hack to force the settlement as failed of the TXs to nuke.
                            forced_txs.insert(tx_hash.clone());
                            blob_metadata
                                .possible_proofs
                                .push((program_id, hyli_output.clone()));
                            block_under_construction
                                .transactions_events
                                .entry(tx_hash.clone())
                                .or_default()
                                .push(TransactionStateEvent::NewProof {
                                    blob_index: hyli_output.index,
                                    proof_tx_hash: bth.clone(),
                                    program_output: hyli_output.program_outputs.clone(),
                                });
                        }
                    }
                }
    ...
    fn verify_hyli_output(
        ...
        if hyli_output.tx_hash != unsettled_tx.hash {
            bail!(
