...
#[test_log::test(tokio::test)]
async fn test_tx_with_hyli_blob_should_have_specific_timeout_after_block445_000() {
...
async fn test_nuke_transaction_forces_target_to_fail() {
    ...
    let nuke_action = NukeTxAction {
        txs: BTreeMap::from([
            (tx2_hash.clone(), vec![hyli_output_tx2]),
    ...
    let proof1 = new_proof_tx(
        &contract_a,
        &make_hyli_output_with_state(tx1.clone(), BlobIndex(0), &[0, 1, 2, 3], &[4, 5, 6]),
        &tx1_hash,
    );
